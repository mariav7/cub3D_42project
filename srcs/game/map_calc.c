...
{
	if(i_data[SIDE] == 0)
		d_data[PERP_WALL_DIST] = (i_data[MAP_X] - player->posX + (1 - i_data[STEP_X]) / 2) /  d_data[RAY_DIR_X];
	else
		d_data[PERP_WALL_DIST] = (i_data[MAP_Y] - player->posY + (1 - i_data[STEP_Y]) / 2) / d_data[RAY_DIR_Y];
